...
ardroneCommandGUI mainWindow;

mainWindow.show();

/*
//Lets see if we can start up roscore
if(startCommand("roscore") != 1)
{
printf("Unable to start roscore\n");
return 1;
}
sleep(5);

if(killCommand("roscore") != 1)
{
printf("Unable to kill roscore\n");
return 1;
}
...
/*
auto buffer = getListOfActiveProcesses();

for(int i=0; i<buffer.size(); i++)
{
if(buffer[i].first.find("roscore") != std::string::npos)
{
system((std::string("kill ") + std::to_string(buffer[i].second)).c_str());
printf("Attempting to kill roscore\n");
}
printf("%s\n", buffer[i].first.c_str());
}
*/
